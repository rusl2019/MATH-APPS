...
CREATE TABLE `pkl_applications` (
  ...
  `seminar_location` varchar(255) DEFAULT NULL,
  `created_at` timestamp NULL DEFAULT current_timestamp(),
...
CREATE TABLE `pkl_assessments` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `application_id` int(11) NOT NULL,
  `assessor_type` enum('lapangan','dosen','dosen_pembimbing') NOT NULL,
...
CREATE TABLE `pkl_workflow` (
  ...
  `status` enum('pending','approved','rejected','done','submitted','scheduled','revision_started','completed') DEFAULT 'pending',
